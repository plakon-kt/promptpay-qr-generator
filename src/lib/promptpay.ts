...
function formatTarget(rawInput: string): { type: TargetType; formattedValue: string } {
    ...
    throw new Error("ข้อมูลไม่ถูกต้อง: ต้องเป็นเบอร์โทรศัพท์ 10 หลัก หรือ เลขบัตรประชาชน 13 หลัก");
...
string {
    ...
    //ถ้ามี amount ให้ format เป็นทศนิยม 2 ตำแหน่งด้วย amount.toFixed(2) แล้วเติมเป็น tag 54
    if (hasAmount) {
